...
public class CourseRoomInteractorImpl implements CourseRoomInteractor {
    ...
    private void notifySuccess(final List<Classroom> classrooms) {
        MainThread.getInstance().post(new Runnable() {
            @Override
            public void run() {
                callback.onLoadCourseRoomsSuccess(classrooms);
            }
        });
    }
    ...
    private void notifyFailure(final ExceptionInfo reason) {
        MainThread.getInstance().post(new Runnable() {
            @Override
            public void run() {
                callback.onLoadCourseRoomsError(reason);
            }
        });
    }
}
